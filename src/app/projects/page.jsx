...
function ProjectCard({ project, index }) {
  return (
    <motion.article
      ...
      whileHover={{ y: -6 }}
      className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl transition"
    >
      <Link href={project.href} className="block">
        <div className="relative w-full aspect-[16/10] overflow-hidden">
          <Image src={project.image} alt={project.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" loading="eager" />
        ...
        <div className="p-5">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-white text-lg font-semibold group-hover:text-purple-300 transition-colors duration-300">{project.title}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {project.tags.map((t) => (
              <span key={t} className="text-[11px] px-2 py-1 rounded-full border border-white/10 bg-white/5 text-white/70">
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">
              Projects We <span className="bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent">Loved Building</span>
            </h1>
            <p className="text-gray-300 text-sm sm:text-base max-w-3xl mx-auto leading-relaxed">
              A selection of recent projects that combine performance, aesthetics, and measurable business results.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between mb-6">
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1 sm:pb-0">
              {categories.map((c) => (
                <CategoryChip key={c.key} label={c.title} count={counts[c.key]} active={active === c.key} onClick={() => setActive(c.key)} />
              ))}
            </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={`${active}-${query}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.25 }}>
              ...
              {filtered.length === 0 && (
                <div className="text-center text-white/70 text-sm mt-10">No projects match your search.</div>
              )}
            </motion.div>
          </AnimatePresence>
        </div>
